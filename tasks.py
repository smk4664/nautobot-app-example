...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_EXAMPLE_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.example.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.example.compose_http_timeout,
        "NAUTOBOT_VER": context.example.nautobot_ver,
        "PYTHON_VER": context.example.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.example.project_name}",
        f'--project-directory "{context.example.compose_dir}"',
    ]

    for compose_file in context.example.compose_files:
        compose_file_path = os.path.join(context.example.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.example.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.example.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.example.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.example.compose_dir, "creds.example.env"
            ),
            os.path.join(context.example.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.example.python_ver
    dockerfile_path = os.path.join(context.example.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.example.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations example"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} example"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("example") / Path("migrations")
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 example/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.example.local):
